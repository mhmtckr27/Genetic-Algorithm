...
public partial class VectorLine {
	...
	public static VectorLine SetLine (Color color, float width, params Vector2[] points) {
		return SetLine (color, 0.0f, width, points);
	}

	public static VectorLine SetLine (Color color, float time, float width, params Vector2[] points) {
		if (points.Length < 2) {
			Debug.LogError ("VectorLine.SetLine needs at least two points");
			return null;
		}
		var line = new VectorLine("Line", new List<Vector2>(points), null, width, LineType.Continuous, Joins.None);
		line.color = color;
		if (time > 0.0f) {
